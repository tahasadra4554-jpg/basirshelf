 ...
 * It keeps the whole app (including the teacher panel) fully usable for
 * local development and preview deployments. The data lives in
 * `data/db.json` and is seeded by `scripts/generate-seed.mjs`.
 */
...
function seedTemplate(): LocalDb {
  return { profiles: [], books: [], sections: [] };
}
...
export const localStore = {
  ...
  async deleteBook(id: string, _actorId?: string): Promise<boolean> {
    ...
    db.sections = db.sections.filter((s) => s.book_id !== id); // cascade
    write(db);
